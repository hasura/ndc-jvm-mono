...
object SnowflakeConfigGenerator : IConfigGenerator {
    ...
    override fun generateConfig(
        ...
        return DSL.using(modifiedJdbcUrl).use { ctx ->
            ...
            LEFT OUTER JOIN (
                SELECT
                    ...
                    object_agg(fks."fk_name", to_variant(fks."constraint")) AS FOREIGN_KEYS
                FROM (
                    SELECT
                ...
                if (fullyQualifyNames)
                    ...
                    GROUP BY foreign_keys."fk_database_name", foreign_keys."fk_schema_name", foreign_keys."fk_table_name", foreign_keys."fk_name", foreign_keys."pk_database_name", foreign_keys."pk_schema_name", foreign_keys."pk_table_name"
                ) AS fks
            ...
            val tables = ctx.fetch(sql).map { row ->
                val primaryKeys = row.get("PK_COLUMNS", String::class.java)
                ...
                val columns = row
                    ...
                    .map { it.copy(is_primarykey = it.name in primaryKeys) }
                ...
                TableSchemaRow(
                    ...
                    columns = columns,
                    pks = columns.filter { it.is_primarykey ?: false }.map { it.name },
