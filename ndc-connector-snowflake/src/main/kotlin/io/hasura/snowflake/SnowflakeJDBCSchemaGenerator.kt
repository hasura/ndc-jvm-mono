...
object SnowflakeJDBCSchemaGenerator : JDBCSchemaGenerator() {
    ...
    override fun mapScalarType(columnTypeStr: String, numericScale: Int?): NDCScalar {
        if (columnTypeStr.uppercase() == "NUMBER" && numericScale == 0) {
            return NDCScalar.INT
        } else {
            val javaSqlType = SnowflakeType.convertStringToType(columnTypeStr)
            return javaSqlTypeToNDCScalar(javaSqlType)
        }
    }
}
